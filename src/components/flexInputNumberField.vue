<template>
    <span class="inputField">
        <span class="inputPrompt">{{configElement.prompt}}</span>
            <span>
                <input ref="inputField" type="number" :size=configElement.fieldSize  @keydown.tab.exact = "tabErr" @keyup.enter="textEntered" v-model="value"/>
            </span>
    </span>
</template>
...
//        debugger;
        ...
        if(typeof(this.configElement.minmax)!='undefined'){
             this.minMax =  this.configElement.minmax.split(':');
        }
...
//        debugger;
        ...
        if(this.configElement.required && this.value==0){
            this.fieldValidated=false;
            alert('this field is required');
        }else if(this.value<this.minMax[0] || this.value>this.minMax[1]){
                this.fieldValidated=false;
                alert("entry must fall within range (>= to "+this.minMax[0]+" and <= "+this.minMax[1]);
        }else{
